            ...
            return webClient.get()
                    ...
                    .onStatus(status -> status.is4xxClientError(), clientResponse ->
                            clientResponse.bodyToMono(String.class)
                                    .flatMap(errorBody -> {
                                        log.error("Booking API 4xx error: {}", errorBody);
                                        return Mono.error(new ExternalApiException("Invalid request to Booking API: " + errorBody));
                                    }))
                    .onStatus(status -> status.is5xxServerError(), clientResponse ->
                            Mono.error(new ExternalApiException("Booking API server error")))
                    .bodyToMono(BookingHotelResponse.class)
                    .timeout(Duration.ofSeconds(30))
                    .block();
        } catch (Exception e) {
            log.error("Error searching hotels from Booking: {}", e.getMessage(), e);
            throw new ExternalApiException("Failed to search hotels from Booking API", e);
        }
    }
}
